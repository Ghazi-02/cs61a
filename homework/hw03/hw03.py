...
def num_eights(pos):
    ...
    True
        ...
        if pos < 10:
            ...
            if pos % 10 == 8:
                return  helper(pos // 10) + 1
            else: 
                return helper(pos// 10)      
...
def pingpong(n):
 ...
 Iteration
    ...
    inc = 1
    while counter < n:        
       if counter % 8 == 0 or num_eights(counter) > 0:                
            if inc == 1:
                inc = -1
                value = value + inc
                counter = counter + 1
    ...
    return value
    """
    def helper(value,counter,inc):
         
                  
            if counter < n:
                if  counter % 8 == 0 or num_eights(counter) > 0:                
                    if inc == 1:
                        return  helper(value - 1, counter + 1,inc = -1)
                    else:                     
                        return helper(value + 1 , counter + 1, inc = 1)
                else:
                    return helper(value + inc, counter + 1,inc)
...
def count_coins(change):
    ...
    def helper(change, max):
        ...
        without_m = helper(change,next_smaller_coin(max))
        with_m = helper(change - max,max)
